...
def knn_classify_batch(X_train: torch.Tensor, y_train: torch.Tensor,
...
def knn_classify_batch_vectorized(X_train: torch.Tensor, y_train: torch.Tensor,
    ...
    # Compute pairwise distance matrix: shape (n_query, n_train)
    # ||a - b||^2 = ||a||^2 + ||b||^2 - 2*a.b
    X_train_sq = torch.sum(X_train ** 2, dim=1).unsqueeze(0)  # (1, n_train)
    X_query_sq = torch.sum(X_query ** 2, dim=1).unsqueeze(1)  # (n_query, 1)
    cross_term = torch.mm(X_query, X_train.T)  # (n_query, n_train)

    dist_sq = X_query_sq + X_train_sq - 2 * cross_term
    distances = torch.sqrt(torch.clamp(dist_sq, min=0))  # Numerical stability
...
def knn_classify_robust(X_train: torch.Tensor, y_train: torch.Tensor,
                        x_query: torch.Tensor, k: int = 3) -> int:
    """
    Robust KNN with edge case handling.
    """
